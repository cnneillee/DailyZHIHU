package com.neil.dailyzhihu.utils;

import android.content.Context;
import android.widget.ImageView;

/**
 * Created by Neil on 2016/4/12.
 */
public class UniversalLoader implements IwrappedLoader {
    ImageLoader imageLoader = null;

    public void initLoader(Object o) {
        this.imageLoader = (ImageLoader) o;
    }

    @Override
    public void loadImage(Context context, ImageView iv, String imgUrl,
                          ImageLoader.OnFinishListener onFinishListener) {
        if (imageLoader == null)
            imageLoader.loadImage(context, iv, imgUrl, onFinishListener);
    }

    @Override
    public void loadImage(ImageView iv, String imgUrl,
                          ImageLoader.OnFinishListener onFinishListener) {
        if (imageLoader == null)
            imageLoader.loadImage(iv, imgUrl, onFinishListener);
    }
}
